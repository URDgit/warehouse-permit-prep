...
- **FIRST CONVERSION — 7/15:** touch #27 (Lusk attempt-two, fired 7/15 8:30 AM) got a reply. **Jonathan Lusk (Premier Fire Consulting) ENGAGED; Fontana demo v1.1 sent to him 7/15** under the replies rule. CSV status: "engaged".
- **7/16 — batch-one breakup (third) touches sent to all 5 principals with demo v1.1 attached** (touches #28–32). **JMC2 (John Cruikshank) replied 9:14 AM — polite close ("will contact if needed"); one-line courtesy reply sent, thread ended → CSV status "closed-warm (keep-on-file)", demo retained by prospect, no further touches ever.** Scoreboard: **32 touches → 2 replies (1 engaged, 1 closed-warm).**
- **CAMPAIGN DOCTRINE (7/16): artifact-led format is the standard.** (1) Every cold draft includes a demo-offer line. (2) Demo sent same-hour on any reply. (3) Demo attached to breakup touches only — never first-touch cold sends. **Rationale: 2/2 replies came from artifact-led formats vs 0/26 from describe-only touches.**
...
- **Portal sweep #2 COMPLETE (Ontario, 7/16):** 168 HPS records (2024–2026 + 2020 bonus) via Claude in Chrome, spot-verified 3 rows. Ontario = **FC- prefix, Fire module, no HPS type filter → month-window sweep method**. Volume **~50–75 HPS/yr ≈ 2–3× Fontana**. Same four consultant firms dominate both cities; **new second tier surfaced for wave three**: Zhen Bao/First Rack Permit (8+ filings, top priority), Larry Smiley/CWS, Javier Verdugo/Design Permit HPS, Carlos Lopez (recategorized from intel-only — 4+ Ontario filings, volume runner), Telgian (national-firm tier, parked). Premier Fire staffer #4 surfaced (Marco Robles); Lynn Seymour's email corrected to permits@triadfire.com per Ontario records. Details: `outreach/ontario-portal-notes.md`.
- **Wave three SENT 7/17 ~10:30 AM** (fresh composes, artifact-led doctrine): Zhen Bao, Larry Smiley, Carlos Lopez, Javier Verdugo → **26 sends to unique recipients, 36 touches total** (26 sends + 5 nudges + 5 breakups). New rows from the raw log: Victor Beltran/VMB Fire (consultant-lane, wave-four candidate), Daniel Ziemer PE (engineer-lane — self-filed re-permit, proven HPS engineer), **Enhao Zhang PE 92778 (engineer-lane PRIORITY — likely the SCED Inc. near-perfect-fit; needs email hunt via SCED site / CA license lookup)**, Jose Rodriguez (intel-only, single filing). Keyani Crow noted on the Storage Solutions row (dealer/referral pool, no separate row).
- **Zhang double-contact 7/17 (touch #37):** DCA license 92778 verified **CLEAR** — identity locked (SCED = Structural Engineering and Design). Contacted via SCED site form (artifact-offer, email-capture close) + LinkedIn connection request with note. CSV status: "contacted-alt-channel"; direct email captured on reply. **37 touches total.**
- **SECOND CONVERSION — 7/17:** **Javier Verdugo (Design Permit HPS) replied to his wave-three send the same day, asking for the demo — Fontana demo v1.1 sent** (same-hour replies rule). CSV status: "engaged". **Scoreboard: 37 touches → 3 replies (2 engaged, 1 closed-warm). Wave three: 1 reply from 4 artifact-led cold sends within a day. Every reply the campaign has ever gotten (3/3) came from an artifact-led format; describe-only remains 0/26.**
- **Portal sweep #3 (Moreno Valley, 7/17):** 34 FHP records 2024–2026 archived in `outreach/moreno-valley-portal-notes.md`. **All applicant/professional data "Not public" — zero prospect extraction possible**; consultant identities likely on cross-referenced BFC- plan-check records. Volume ~12–15/yr (smaller than Fontana/Ontario); national tenants dominate (Harbor Freight 548,000 SF AM&M review = largest scope seen on any portal). **Verdict: delivery jurisdiction, not a prospecting jurisdiction. Sweep-method rule: spot-check one record's contact visibility BEFORE sweeping any new city (Rancho/Jurupa included).**
- **Campaign queue:** convert the Lusk engagement; await Zhang reply (form/LinkedIn); wave four (Beltran + Ziemer); optional remaining sweeps — Rancho Cucamonga, Jurupa Valley (visibility spot-check first).
