...
- **Outreach LAUNCHED July 6, 2026 — 13 total cold emails sent across two lanes** (all sent manually from uriel@permitwright.com; free-sample CTA). **Engineer lane (12, pain-first draft):** 5 on 7/6 to structural engineering principals (Storage Rack Engineering, Burke Structural, T&B Engineering, YA Creative, JMC2) + 7 on 7/7 to firm inboxes (RME Structural, AAA Engineering Design, Blue Horizon, Riverside Engineering, NTS Engineering & Design, A to Z Engineering, AEC Moreno Design). **Consultant lane (1, consultant-lane draft v1 — see `outreach/draft-consultant-lane.md`):** Premier Fire Consulting (kevin@premier-fire.net) on 7/7; Jonathan Lusk is the attempt-two contact if no reply by ~7/14. Tracked in `outreach/prospects.csv` (`sent_date`/`status` columns). Business state is **"live service awaiting first replies," not pre-launch**.
...
- **Fontana demo sample** (`deliverables/sample-package-fontana-demo.docx` — fictional project, for generic "show me a sample" replies): **Demo status: DRAFT — not cleared to send until Uriel's citation verification pass is complete** (worksheet: `deliverables/verify-pass.md`). **Deployment rule: demo attaches only to replies, never to cold sends.** After the pass, the "verify at issue" notes come out and the demo is finalized.
